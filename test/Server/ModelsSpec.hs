...
spec = do
  ...
  describe "makeEmptyBoard Spec" $ do
    ...
    it "Amount property" $ property $ \n ->
      Map.size (boardCells
                $ makeEmptyBoard (BoardSize (fromIntegral (n :: Word8)))) == fromIntegral n^2
  ...
  describe "makeRandomMove Spec" $ do
    ...
    it "make 2 random moves" $ property $ \n -> do
      ...
      let (gen1, gen2) = (mkSMGen $ fromIntegral boundedN,
                          mkSMGen $ fromIntegral boundedN + 1)

      let board = makeEmptyBoard $ BoardSize boundedN
      let (Right updBoard) =
            (makeRandomMove gen1 PlayerX board) >>= (makeRandomMove gen2 Player0)
